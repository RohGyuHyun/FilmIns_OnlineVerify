...
20, 15, 720, 0
기능 추가
 - AMIF(Align fail), STDF(Align fail-no pattern), AMPL(Align limit error), PCND(Punching불량), NOCP(No chip)
   해당 NG 발생 시 Main Display에 NG 이름 만 표시, LT(0,0) ~ RB(100,100) 확대
 - Verify 처리 후 NG Veiw에 확대 된 영상 저장
 - AMIF(Align fail), STDF(Align fail-no pattern), AMPL(Align limit error), PCND(Punching불량), NOCP(No chip)
   해당 NG type은 임시로 전체 이미지 저장
//////////////////////////////////////////////////////////////////////////////////



================================================================================
    MFC 라이브러리 : FilmIns_OnlineVerify 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 FilmIns_OnlineVerify 응용 프로그램을 
만들었습니다.  이 응용 프로그램은 MFC(Microsoft Foundation Classes)의
기본적인 사용법을 보여 주며 응용 프로그램 작성을 위한 출발점을 제공합니다.

이 파일에는 FilmIns_OnlineVerify 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

FilmIns_OnlineVerify.vcproj
    응용 프로그램 마법사를 사용하여 생성된 VC++ 프로젝트의 주 프로젝트 파일입니다.
    파일을 생성한 Visual C++ 버전에 대한 정보와
    응용 프로그램 마법사를 사용하여 선택한 플랫폼, 구성 및 프로젝트
    기능에 대한 정보가 들어 있습니다.

FilmIns_OnlineVerify.h
    응용 프로그램의 기본 헤더 파일입니다.  여기에는 다른 프로젝트 관련
    헤더(Resource.h 포함)가 들어 있고 CFilmIns_OnlineVerifyApp 응용 프로그램
    클래스를 선언합니다.

FilmIns_OnlineVerify.cpp
    응용 프로그램 클래스 CFilmIns_OnlineVerifyApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.

FilmIns_OnlineVerify.rc
...
res\FilmIns_OnlineVerify.ico
    응용 프로그램의 아이콘으로 사용되는 아이콘 파일입니다.  이 아이콘은
    주 리소스 파일인 FilmIns_OnlineVerify.rc에 의해 포함됩니다.

res\FilmIns_OnlineVerify.rc2
...
FilmIns_OnlineVerifyDlg.h, FilmIns_OnlineVerifyDlg.cpp - 대화 상자
    이 파일에는 CFilmIns_OnlineVerifyDlg 클래스가 들어 있습니다.  이 클래스는
    응용 프로그램의 주 대화 상자 동작을 정의합니다.  대화 상자의 템플릿은
    Microsoft Visual C++에서 편집할 수 있는 FilmIns_OnlineVerify.rc에 있습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일인 FilmIns_OnlineVerify.pch와
